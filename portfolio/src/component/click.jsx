...
export default function SocialSection() {
  ...
  // State to track if we should show the mask
  const [isMobile, setIsMobile] = useState(false)

  // useEffect to handle window resizing safely in Next.js/React
  useEffect(() => {
    const handleResize = () => {
  ...
  return (
    <footer className="bg-[#c8f550] relative overflow-hidden min-h-screen px-3 sm:px-4 md:px-8 pb-4 sm:pb-6 flex flex-col justify-end">
      ...
      <div className="relative flex-1 flex flex-col w-full max-w-[1688px] mx-auto mt-4 sm:mt-8 md:mt-12 z-10 min-h-[auto] sm:min-h-[70vh] md:min-h-[80vh]">
        ...
        {/* Background Layer with Conditional Mask */}
        <div
          className="absolute inset-0 w-full h-full z-0 bg-[#282c20] overflow-hidden rounded-[2rem] md:rounded-none"
          style={{
            // Apply mask only if NOT mobile
            maskImage: !isMobile ? `url("${footerMaskSVG}")` : "none",
            WebkitMaskImage: !isMobile ? `url("${footerMaskSVG}")` : "none",
            maskSize: "100% 100%",
        ...
        <div className="relative z-20 flex flex-col h-full px-3 sm:px-6 md:px-12 lg:px-24 py-6 sm:py-10 md:py-12 lg:py-20">
          ...
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-4 sm:gap-6 md:gap-8 items-stretch">
            ...
            {/* Left Column (Pages) - Hidden on mobile/tablet, shown on LG */}
            <div className="hidden lg:flex col-span-3 text-center flex-col justify-center">
              <h4 className="font-black text-xs uppercase mb-6 text-white/40 tracking-[0.2em]">PAGES</h4>
              <ul className="space-y-2">
                {["Home-ish", "Builds", "About"].map((item) => (
                  <li key={item}>
                    <a href={`#${item.toLowerCase().replace(" ", "-")}`} className="text-white font-bold text-2xl uppercase hover:text-[#c8f550] transition">
                      {item}
                    </a>
                  </li>
                ))}
                <li><a href="#store" className="text-[#c8f550] font-black text-2xl uppercase hover:text-white transition">STORE</a></li>
              </ul>
            </div>
            ...
            <div className="lg:col-span-6 flex items-center justify-center">
              ...
              >
                <div>
                  ...
                  <input type="text" name="name" required placeholder="Your name" className="bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white w-full outline-none focus:border-[#c8f550]" />
                </div>
                <div>
                  <label className="text-xs uppercase tracking-widest text-white/60">Email</label>
                  <input type="email" name="email" required placeholder="you@example.com" className="bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white w-full outline-none focus:border-[#c8f550]" />
                </div>
                <div>
                  <label className="text-xs uppercase tracking-widest text-white/60">Message</label>
                  <textarea name="message" rows={3} required placeholder="Tell me about your project..." className="bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white w-full resize-none outline-none focus:border-[#c8f550]" />
                </div>
                <motion.button
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  type="submit"
                  className={`w-full bg-[#c8f550] text-black font-bold uppercase tracking-wider py-3 rounded-full transition ${status === "sending" ? "cursor-wait" : ""}`}
            ...
            <div className="lg:col-span-3 text-center flex flex-col justify-center mb-8 sm:mb-0">
              <h4 className="text-xs uppercase tracking-[0.2em] mb-6 text-white/40 font-black">Follow</h4>
              <ul className="space-y-2">
                {["GitHub", "LinkedIn", "Twitter"].map((p) => (
                  <li key={p}>
                    <a href={socialLinks[p]} target="_blank" rel="noopener noreferrer" className="block font-bold uppercase text-2xl text-white hover:text-[#c8f550] transition">
                      {p}
                    </a>
                  </li>
                ))}
